...
do
    ...
    case "$process" in
        "fusion")
            params="$params --tab -e \"ssh -t ${machine} 'cd ${start_dir}; source activate py27gpu; python -m components.fusion.fusion_server; bash;'\" --title ${i}"
            ;;
        "lh")
            params="$params --tab -e \"ssh -t ${machine} 'cd ${start_dir}; source activate py27gpu; export CUDA_VISIBLE_DEVICES=${device}; python -m components.handRecognition.depth_client LH; bash;'\" --title ${i}"
            ...
            ;;
        "rh")
            params="$params --tab -e \"ssh -t ${machine} 'cd ${start_dir}; source activate py27gpu; export CUDA_VISIBLE_DEVICES=${device}; python -m components.handRecognition.depth_client RH; bash;'\" --title ${i}"
            if [ "$single_machine" = yes ]
            then
                ((device++))
            fi
            ;;
        "head")
            params="$params --tab -e \"ssh -t ${machine} 'cd ${start_dir}; source activate py27gpu; export CUDA_VISIBLE_DEVICES=${device}; python -m components.headRecognition.head_client; bash;'\" --title ${i}"
            if [ "$single_machine" = yes ]
            then
                ((device++))
            fi
