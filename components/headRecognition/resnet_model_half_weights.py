...
class ResNet(object):
    ...
    def __init__(self, hps, mode):
        ...
        """
        self.hps = hps
        self._images = tf.placeholder(tf.float32, [hps.batch_size, 128,128, 30])
        self.labels = tf.placeholder(tf.float32, [hps.batch_size, hps.num_classes])
        self.mode = mode
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('init'):
            x = self._images
            print x
            x = self._conv('init_conv', x, 7, 30, 64, self._stride_arr(2))
